# -*- coding: utf-8 -*-
"""
Illuminati Terminal v23.3 - The "AI Fortress" Build
FIXES:
- LLM Crash: Replaced exact model matching with "Fuzzy Search".
- Logic: Scans your available models for keywords ('flash', '1.5', 'pro') to find a working one.
- Data: Retains the "Hybrid Omni" scan (News + 500 Stocks) for maximum coverage.
"""
...
# Without this list, you only get 5-10 stocks mentioned in today's news.
NIFTY_500_UNIVERSE = [
...
class GeminiBrain:
    ...
    def generate_narrative(self, df_summary):
        ...
        # KEY FIX: FUZZY MATCHING FOR MODELS
        try:
            # 1. Ask Google what models are ACTUALLY available to this key
            all_models = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
            
            # 2. Look for keywords in the available models
            # Priority: 'flash' > '1.5-pro' > 'gemini-pro'
            keywords = ['flash', '1.5-pro', 'gemini-pro', '1.0-pro']
            chosen_model = None
            ...
            if not chosen_model:
                return "AI Error: No text models found."
...
def run_illuminati(interactive=False, tickers_arg=None):
    ...
    print("\n" + "="*80)
    print(f"👁️ ILLUMINATI TERMINAL v23.3 (AI FORTRESS) | {current_time}")
    print("="*80)
    ...
    combined_tickers = list(set(news_tickers + NIFTY_500_UNIVERSE))
